...
func (a *AegisServer) StartSSE(cfg SSEConfig) error {
	...
	a.mu.RLock()
	hasKeys := len(a.apiKeyPrefixes) > 0
	a.mu.RUnlock()
	if !hasKeys {
		return fmt.Errorf("SSE transport requires at least one api_keys entry in the API configs")
	}
	...
	fmt.Fprintf(os.Stderr, "[AEGIS] %s SSE listening at %s%s/<mcp-prefix>/sse\n", schemeLabel, strings.TrimRight(cfg.BaseURL, "/"), cfg.BasePath)
	go a.runWatcher()
...
func (a *AegisServer) wrapSSEAuth() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		prefix, next, ok := a.handlerForRequestPath(r.URL.Path)
		if !ok {
			http.NotFound(w, r)
			return
		}

		ctx, err := a.authenticatedContextForRequest(r, prefix)
		if err != nil {
...
func (a *AegisServer) authenticatedContextForRequest(r *http.Request, requestedPrefix string) (context.Context, error) {
	...
	if sessionID != "" {
		...
		if access.Prefix != requestedPrefix {
		...
		prefix, ok := a.aliasForAPIKey(access.Token)
		if !ok {
		...
		return withAccessContext(r.Context(), access.Token, prefix), nil
	}
	...
	prefix, ok := a.aliasForAPIKey(token)
	if !ok {
	...
	return withAccessContext(r.Context(), token, prefix), nil
}
...
func (a *AegisServer) handlerForRequestPath(path string) (string, http.Handler, bool) {
	...
	a.mu.RLock()
	prefix, ok := a.pathAliases[parts[0]]
	a.mu.RUnlock()
	if !ok {
		return "", nil, false
	}

	return prefix, a.sseHandlerForPrefix(prefix), true
}

func (a *AegisServer) sseHandlerForPrefix(prefix string) http.Handler {
	a.mu.RLock()
	if handler, exists := a.sseServers[prefix]; exists {
		a.mu.RUnlock()
		return handler
	}
	cfg := a.sseCfg
	...
	basePath := endpointBasePath(cfg.BasePath, prefix)
	...
	a.mu.Lock()
	defer a.mu.Unlock()
	if existing, exists := a.sseServers[prefix]; exists {
		return existing
	}
	a.sseServers[prefix] = handler
	return handler
}
...
func setBearerChallenge(w http.ResponseWriter, err error) {
	...
	w.Header().Set(
		"WWW-Authenticate",
		fmt.Sprintf(`Bearer realm="aegis-api-mcp", error="%s"`, errorCode),
	)
}
